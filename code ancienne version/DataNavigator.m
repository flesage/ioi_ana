...
h.ui.EventSaveCB = uicontrol('Style','checkbox','Parent', h.ui.EventsPan,...
    ...
    'String','Save Events','Value',0);
h.ui.EventbaddatCB = uicontrol('Style','checkbox','Parent', h.ui.EventsPan,...
    'Units', 'normalized', 'Position',[0.45 0.90 0.25 0.075],...
    'String','Inverse Master/Slave','Value',0);
...
h.ui.SetPS = uicontrol('Style','popupmenu','Parent', h.ui.PStimL,...
    'Units', 'normalized', 'Position',[0.1 0.1 0.8 0.8],...
    'String',{'0s', '2s', '5s', '10s','15s'}, 'Value', 1, 'Callback', @setPreStimLength);
...
%%% area finder
h.ui.dispFtresh = uipanel('Parent', h.ui.fig, 'Title','find threshold','FontSize',12,...
             'Position',[.650 .125 .150 .105]);
         
h.ui.FtreshButton = uicontrol('Style','pushbutton','Parent', h.ui.dispFtresh,...
    'Units', 'normalized', 'Position',[0.1 0.1 0.8 0.8],'string','Find'...
    ,'Callback',@gui_handle);
...
%%%%%%%%%%%%%%%
    ...
    function setPreStimLength(Src,~,~)
        ...
        elseif( sID == 5 )
            h.data.MasterStim.PreStimLength = 15;
        end
        OpenFolder(Src);
    end
    ...
    function exportXLS(~,~,~)
        ...
        delete(ExportDlg);
        disp(['Done exporting Excel file ' h.paths.FolderName]);
    end
    ...
    function OpenFolder(Src, ~, ~)
        ...
        delete(h.ui.EventsDispPan.Ax);
        h.ui.EventsDispPan.Ax = axes('Parent', h.ui.EventsDispPan.Container, 'Position', ...
                [0.175 0.15 0.72 0.85]);
        delete(h.ui.EventsMeanPan.Ax);
        h.ui.EventsMeanPan.Ax = axes('Parent', h.ui.EventsPan, 'Position', ...
                [0.15 0.15 0.825 0.325]);
        ...
        if(  exist(h.paths.MasterStimProto, 'file') )
            ...
            h.data.MasterStim.NbStim = h.data.MasterStim.NbStim;
            sID = get(h.ui.SetPS, 'Value');
            if( sID == 1 )
                h.data.MasterStim.PreStimLength = 0.11;
            elseif( sID == 2 )
            ...
            elseif( sID == 5 )
                h.data.MasterStim.PreStimLength = 15;
            elseif( sID == 6 )
                h.data.MasterStim.PreStimLength = 20;
            end
        ...
        if(  exist(h.paths.SlaveStimProto, 'file') )
            ...
            h.data.SlaveStim.NbStim = h.data.SlaveStim.NbStim;
            sID = get(h.ui.SetPS, 'Value');
            if( sID == 1 )
            ...
            elseif( sID == 5 )
                h.data.SlaveStim.PreStimLength = 15;
            elseif( sID == 6 )
                h.data.SlaveStim.PreStimLength = 20;
            end
    ...
    function MoveEventsDisp(~,~,~)
        ...
        if(h.flags.SlaveStim)            
            h.ui.EventsDispPan.dispAx.zeroline = line(h.ui.EventsDispPan.Ax,[0  0], [min(d) max(d)], 'Color', 'g', 'LineStyle','--');
            h.ui.EventsDispPan.dispAx.Masterstimline= line(h.ui.EventsDispPan.Ax,[h.data.MasterStim.StimLength h.data.MasterStim.StimLength],[min(d) max(d)],'Color', 'r', 'LineStyle','--');
            h.ui.EventsDispPan.dispAx.Slavestimline= line(h.ui.EventsDispPan.Ax, [h.data.ssdiff+h.data.SlaveStim.StimLength h.data.ssdiff+h.data.SlaveStim.StimLength],[min(d) max(d)],'Color', 'm', 'LineStyle',':');
            h.ui.EventsDispPan.dispAx.Slavezeroline = line(h.ui.EventsDispPan.Ax,[h.data.ssdiff h.data.ssdiff],[min(d) max(d)],'Color', 'b', 'LineStyle',':');
        else
    ...
    function PopulateEvntsDisplay(~, ~, ~)
        badset = get(h.ui.EventbaddatCB,'Value');
         if( isempty(h.data.ROIs) )
             return;
         end
        ...
        if( ValidateEvntSrc(SelectedSrc, SelectedROI) )
            ...
            if(badset)
                eLen = floor(h.data.AcqFreq*...
                    (h.data.SlaveStim.StimLength + h.data.SlaveStim.InterStim_min));
            else
                eLen = floor(h.data.AcqFreq*...
                    (h.data.MasterStim.StimLength + h.data.MasterStim.InterStim_min));
            end
            ...
            if(badset)
                T = linspace(-h.data.SlaveStim.PreStimLength, h.data.SlaveStim.StimLength + h.data.SlaveStim.InterStim_min - h.data.SlaveStim.PreStimLength, eLen);
                h.data.EventBuf = zeros(h.data.SlaveStim.NbStim + 1, eLen, 'single');
            else
                T = linspace(-h.data.MasterStim.PreStimLength, h.data.MasterStim.StimLength + h.data.MasterStim.InterStim_min - h.data.MasterStim.PreStimLength, eLen);
            ...
            if(get(h.ui.EventSaveCB,'Value'))
               if(strcmp(SelectedSrc,'Red') || strcmp(SelectedSrc,'Green') ||strcmp(SelectedSrc,'Yellow'))
                ...
                prompt = {'Colormap minimum:','Colormap maximum:'};
                dlg_title = 'Colormap Limits';
                defaultans = {num2str(cmValues(1)),num2str(cmValues(2))};
                num_lines = 1;
                answer = inputdlg(prompt,dlg_title,num_lines,defaultans);
                cmValues(1) =  str2double(answer{1});
                cmValues(2) =  str2double(answer{2});
            end
            for indE = 1:h.data.MasterStim.NbStim   
                if( isHbT == 0 )
                    d = data.Data( (length(h.data.Map(:))*(StartPts(indE) - 1) + 1):(length(h.data.Map(:))*(StartPts(indE) +eLen - 1)));
                else
                    d1 = h.data.hoDatPtr.Data( (length(h.data.Map(:))*(StartPts(indE) - 1) + 1):(length(h.data.Map(:))*(StartPts(indE) +eLen - 1)) );
                    d2 = h.data.hrDatPtr.Data( (length(h.data.Map(:))*(StartPts(indE) - 1) + 1):(length(h.data.Map(:))*(StartPts(indE) +eLen - 1)) );
                    d = d1+d2;
                end
                ...
                % save each event timecourse
                if(get(h.ui.EventSaveCB,'Value'))
                    fig_path = char(strcat(sStr{sID}, '-', rStr{rID} ,' Event' ,num2str(indE)));
                    im_path = [tc_path fig_path];
                    if(~isfolder(im_path))
                        mkdir(im_path)
                    end
                    nb_pict = 0;
                    last = -1;
                    for indT = 1:eLen
                        if( mod(round(T(indT)),2) == 0 && round(T(indT))~=last)
                            fig = figure('visible','off');
                            last = round(T(indT));
                            nb_pict = nb_pict + 1;
                            hold on
                            imagesc(d_im(:,:,indT),cmValues);
                            axis off;
                            colormap jet;
                            title_string = char(strcat(num2str(round(T(indT))))); 
                            title(title_string, 'Units', 'normalized', 'Position', [0.5, -0.1, 0]);
                            image_path = [im_path filesep 't_' num2str(nb_pict)];
                            print(fig,image_path,'-djpeg');
                            delete(fig);
                        end
                        if(nb_pict == 20)
                            break;
...
%                     ha=get(gcf,'children');
%                     for indw = 1:length(ha)
%                         if (indw < 11)
%                         set(ha(length(ha)-indw+1),'position',[0.07*indw .5 .07 .1]);
%                         hold on;
%                         else
%                         set(ha(length(ha)-indw+1),'position',[0.07*(indw-10) .30 .07 .1]);
%                         hold on
%                         end
%                     end
%                     hold off
...
%                     colorbar('Position', [hp4(1)+hp4(3)+0.02  0.5  0.01  hp4(4)]);
            ...
            % save mean events timecourse 
            d_im_mean = mean(d_mean,4);
            if(get(h.ui.EventSaveCB,'Value'))
                filepath = [tc_path filesep 'mean_Events'];
                if(~isfolder(filepath))
                    mkdir(filepath);
                end
%                 file_name = char(strcat(SelectedSrc,'_mean_Events'));
%                 fig = figure('visible','off','units','normalized','outerposition',[0 0 1 1]);
                nb_pict = 0;
                last = -1;
                for indT = 1:eLen
                    test = mod(round(T(indT)),2);
                    if( test == 0 && round(T(indT))~=last)
                        fig = figure('visible','off');
                        last = round(T(indT));
                        nb_pict = nb_pict + 1;
%                         subplot(1,12,nb_pict);
                        hold on
                        imagesc(d_im_mean(:,:,indT),cmValues);
                        axis off;
                        colormap jet;
                        colorbar
                        title_string = char(strcat(num2str(round(T(indT)))));  
                        title(title_string, 'Units', 'normalized', 'Position', [0.5, -0.1, 0]);
                        im_path = [filepath filesep 'mean_t' num2str(nb_pict)];
                        print(fig,im_path,'-djpeg');
                        delete(fig);
                    end
                    if(nb_pict == 20)
                        break;
...
%                 delete(fig);
    ...
    function MeanRecalculation()
        ...
        if(badset)
             T = linspace(-h.data.SlaveStim.PreStimLength, h.data.SlaveStim.StimLength + h.data.SlaveStim.InterStim_min - h.data.SlaveStim.PreStimLength, length(d));
        else
    ...
    function RefreshMapDisplay(~,~,~)
        ...
        cmap = gray(64);
        imagesc(h.ui.ROIsMap, round(h.data.Map*64), 'AlphaData', 1,'clipping','off');
        colormap(h.ui.ROIsMap, cmap);
        axis(h.ui.ROIsMap, 'off');
        h.ui.ROIsMap.DataAspectRatio = [1 1 1];
        ...
        for indR = 1:NbRois
            ...
            h.ui.MskAxes(indR).DataAspectRatio = [1 1 1];
        end 
    end
...
% GUI for area parameters selection and functions
    function gui_handle(~,~,~)
        ch_list = get(h.ui.ChannelSelector,'String');
        color_list = {'blue', 'black', 'red', 'green', 'yellow', 'cyan', 'magenta','white'};
        h_area.fig = figure('Name', 'Area parameters', 'Position', [100 25 350 400]);
        h_area.tresh1_edit = uicontrol('Parent',h_area.fig,...
            'style','edit','Units', 'normalized','Position',[0.40 0.75 0.2 0.1],'string', '70');
        h_area.tresh1_string = uicontrol('Parent',h_area.fig,...
            'style','text','Units', 'normalized','Position',[0.30 0.85 0.4 0.05],'string', 'Highest treshold (%)');
        h_area.tresh2_edit = uicontrol('Parent',h_area.fig,...
            'style','edit','Units', 'normalized','Position',[0.40 0.55 0.2 0.1], 'string', '50');
        h_area.tresh2_string = uicontrol('Parent',h_area.fig,...
            'style','text','Units', 'normalized','Position',[0.30 0.65 0.4 0.05],'string', 'Med treshold (%)');
        h_area.tresh3_edit = uicontrol('Parent',h_area.fig,'style',...
            'edit','Units', 'normalized','Position',[0.40 0.35 0.2 0.1],'string', '30');
         h_area.tresh3_string = uicontrol('Parent',h_area.fig,...
            'style','text','Units', 'normalized','Position',[0.30 0.45 0.4 0.05],'string', 'Lowest treshold (%)');
        h_area.continue_pb = uicontrol('Parent',h_area.fig,'style',...
            'pushbutton','Units', 'normalized','Position',[0.40 0.15 0.2 0.1],...
            'string','confirm','callback',@findAreas);
        h_area.tresh1_clist = uicontrol('Parent',h_area.fig,...
            'style','popupmenu','Units', 'normalized','Position',[0.70 0.73 0.2 0.1],'string', color_list);
         h_area.tresh1_cstring = uicontrol('Parent',h_area.fig,...
            'style','text','Units', 'normalized','Position',[0.65 0.83 0.3 0.05],'string', 'Color max treshold');
        h_area.tresh2_clist = uicontrol('Parent',h_area.fig,...
            'style','popupmenu','Units', 'normalized','Position',[0.70 0.53 0.2 0.1],'string', color_list);
         h_area.tresh2_cstring = uicontrol('Parent',h_area.fig,...
            'style','text','Units', 'normalized','Position',[0.65 0.63 0.3 0.05],'string', 'Color med treshold');
        h_area.tresh3_clist = uicontrol('Parent',h_area.fig,...
            'style','popupmenu','Units', 'normalized','Position',[0.70 0.33 0.2 0.1],'string', color_list);
         h_area.tresh3_cstring = uicontrol('Parent',h_area.fig,...
            'style','text','Units', 'normalized','Position',[0.65 0.43 0.3 0.05],'string', 'Color max treshold');
        h_area.chstring = uicontrol('Parent',h_area.fig,...
            'style','text','Units', 'normalized','Position',[0.025 0.9 0.2 0.05],'string', 'Channel');
        h_area.chlist = uicontrol('Parent',h_area.fig,...
            'style','popupmenu','Units', 'normalized','Position',[0.025 0.80 0.2 0.1],'string', ch_list);
        set(h_area.tresh1_clist,'value',3);
        set(h_area.tresh2_clist,'value',5);
        set(h_area.tresh3_clist,'value',4);
    end

    function findAreas(~,~,~)
        
        if(~isfolder([h.paths.FolderName filesep 'activatedAreas']))
            mkdir([h.paths.FolderName filesep 'activatedAreas'])
        end
        set(h_area.fig,'visible','off');
        tresh1 = str2num(get(h_area.tresh1_edit,'string'));
        tresh2 = str2num(get(h_area.tresh2_edit,'string'));
        tresh3 = str2num(get(h_area.tresh3_edit,'string'));
        ch_list = get(h_area.chlist,'String');
        numChan = get(h_area.chlist,'Value');
        SelectedSrc = ch_list{numChan};
        ...
        switch(SelectedSrc)
            ...
            case 'HbT'
                noload = 1;
                temp_dat1 = memmapfile([h.paths.FolderName filesep 'HbO.dat'], 'Format', 'single');
                temp_dat2 = memmapfile([h.paths.FolderName filesep 'HbR.dat'], 'Format', 'single');
                temp_dat = temp_dat1.Data+temp_dat2.Data;
            otherwise 
                filename = SelectedSrc;
        end
        
        if(~noload)
            temp = memmapfile([h.paths.FolderName filesep filename '.dat'], 'Format', 'single');
            temp_dat = temp.Data;
        end
        
        %select and load brain mask
        SelectBrain(h.paths.FolderName);
        
        % color selection for each treshold
        ctresh1 = get(h_area.tresh1_clist,'value');
        ctresh2 = get(h_area.tresh2_clist,'value');
        ctresh3 = get(h_area.tresh3_clist,'value');
        
        [vect_ctresh1,alpha_ctresh1]= ColorSelect(ctresh1);
        [vect_ctresh2,alpha_ctresh2]= ColorSelect(ctresh2);
        [vect_ctresh3,alpha_ctresh3]= ColorSelect(ctresh3);
        
        % process
        waitdialog = dialog('Position',[500 500 250 150],'Name','Computing...');
            uicontrol('Parent', waitdialog, 'Style','text',...
                'Position',[20 80 210 40], 'String', 'Computing Areas. Please wait...');
            pause(0.1);
        
        eLen = floor(h.data.AcqFreq*...
                (h.data.MasterStim.StimLength + h.data.MasterStim.InterStim_min));
        
        if( ~isempty(strfind(SelectedSrc, 'Flow')) )
                StartPts = h.data.F_eflag;
            else
                StartPts = h.data.H_eflag;
        end
        ...
        for indE = 1:h.data.MasterStim.NbStim   
           d = temp_dat((length(h.data.Map(:))*(StartPts(indE) - 1) + 1):(length(h.data.Map(:))*(StartPts(indE) +eLen - 1)));
           dat_set = reshape(d,size(h.data.Map,1),size(h.data.Map,2),[]);
           dat_set_f = imfilter(dat_set, fspecial('gaussian',5,3),'same','symmetric').*repmat(h.data.ROI.brainmask,1,1,size(dat_set,3));
           max_val = max(dat_set_f(:));
           
            for indi = 1:size(dat_set_f,3)
                nb_val_t1(indi) = nnz(dat_set_f(:,:,indi).*h.data.ROI.brainmask > tresh1*max_val/100);
            end
            
            % find which image has the greatest response and plot ROIs for
            % each tresholds
            [max_pos_t1,indx_t1] = max(nb_val_t1);
            roi_tresh1 = dat_set_f(:,:,indx_t1).*h.data.ROI.brainmask > tresh1*max_val/100;
            roi_tresh2 = dat_set_f(:,:,indx_t1).*h.data.ROI.brainmask > tresh2*max_val/100;
            roi_tresh3 = dat_set_f(:,:,indx_t1).*h.data.ROI.brainmask > tresh3*max_val/100;
            total_roi = roi_tresh1 + roi_tresh2 + roi_tresh3;
            
            nb_pixels_tresh1(indE) = nnz(roi_tresh1);
            nb_pixels_tresh2(indE) = nnz(roi_tresh2);
            nb_pixels_tresh3(indE) = nnz(roi_tresh3);
            nb_pixels_total(indE) = nnz(h.data.ROI.brainmask);
            events_nb(indE) = indE;
            
            % Making figure and draw ROIs on image
            fig = figure('visible','off');
            brain_cmap = gray(64);
            brain_ax = axes('Parent',fig,'Position',[0.05 0.05 0.75 0.92]);
            imagesc(brain_ax,round(h.data.Map*64).*h.data.ROI.brainmask,'AlphaData',1);
            colormap(brain_ax,brain_cmap);
            axis(brain_ax,'off');
            tresh1_ax = axes('Parent',fig,'Position',[0.05 0.05 0.75 0.92]);
            imagesc(tresh1_ax,roi_tresh1,'AlphaData',total_roi.*alpha_ctresh1);
            colormap(tresh1_ax,cat(1,[0 0 0],vect_ctresh1));
            axis(tresh1_ax,'off');
            tresh2_ax = axes('Parent',fig,'Position',[0.05 0.05 0.75 0.92]);
            imagesc(tresh2_ax,roi_tresh2 - roi_tresh1 ,'AlphaData',(roi_tresh2 - roi_tresh1).*alpha_ctresh2);
            colormap(tresh2_ax,cat(1,[0 0 0],vect_ctresh2));
            axis(tresh2_ax,'off');
            tresh3_ax = axes('Parent',fig,'Position',[0.05 0.05 0.75 0.92]);
            imagesc(tresh3_ax,(roi_tresh3 - roi_tresh2 - roi_tresh1),'AlphaData',...
                (roi_tresh3 - roi_tresh2 - roi_tresh1).*alpha_ctresh3);
            colormap(tresh3_ax,cat(1,[0 0 0],vect_ctresh3));
            axis(tresh3_ax,'off');
            hold off
            print(fig,[h.paths.FolderName filesep 'activatedAreas' filesep SelectedSrc '_s' num2str(indE) '.jpg'],'-djpeg');
            delete(fig);
            
        end
        strings = {'Events';'treshold max';'treshold med';'treshold min';'ROI'};
        tresh_values = [0;tresh1;tresh2;tresh3; 0];
        nb_pixels = [events_nb;nb_pixels_tresh1;nb_pixels_tresh2;nb_pixels_tresh3; nb_pixels_total];
        xl_tbl = table(strings,tresh_values,nb_pixels);
        writetable(xl_tbl,[h.paths.FolderName filesep 'activatedAreas' filesep SelectedSrc '.xlsx'],'Sheet',1,'Range','D4');
        clear h_area;
        delete(waitdialog);
        
    end

    function [vect_ctresh,alpha_ctresh]= ColorSelect(ctresh)
        switch(ctresh)
        case 1
            vect_ctresh = [0 0 1];
            alpha_ctresh = 0.25;
