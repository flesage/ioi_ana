...
StimON = find((AnalogIN(1:(end-1), sChan) < 2) & (AnalogIN(2:end, sChan) >= 2))+1;
...
if(~isempty(StimON) && sChan == 1)
    camTrigChan = 8;
else
    camTrigChan = 1;
end

CamTrig = find((AnalogIN(1:(end-1),1) < 2) & (AnalogIN(2:end,1) >= 2))+1;

if( ~isempty(StimON) )
    Period = median(StimON(2:end)-StimON(1:(end-1)))/10000;
    Freq = 1/Period;
    Width = sum(AnalogIN(StimON(1):StimON(2),sChan) >2)/(Period*10000);
    
    StimLim = find(diff(StimON)>20000);
    NbStim = length(StimLim)+1;
    if( NbStim == length(StimON) ) %Single Pulse trigged Stims
        StimLim = find((AnalogIN(1:(end-1), sChan) > 2) & (AnalogIN(2:end, sChan) <= 2))+1;
