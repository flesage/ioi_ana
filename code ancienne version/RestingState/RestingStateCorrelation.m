...
%         colormap jet;
    ...
    function Process(~,~,~)
        delete(ui_fig);
        channel_list = {'HbO','HbR','HbT','Flow','Yellow','Red','Green'};
        ...
        for i=1:nFolders
            ...
            if (h.flags.IsThereHbO == true && sum(ismember(indx,1)))
                Correlation('HbO',dataPaths(i,1));
                waitbar((7*(i-1)+1)/(7*nFolders),k);
            end
            if (h.flags.IsThereHbR == true && sum(ismember(indx,2)))
                Correlation('HbR',dataPaths(i,1));
                waitbar((7*(i-1)+2)/(7*nFolders),k);
            end
            
            if (h.flags.IsThereHbO == true && h.flags.IsThereHbR == true && sum(ismember(indx,3)))
                Correlation('HbT',dataPaths(i,1));
                waitbar((7*(i-1)+3)/(7*nFolders),k);
            end
            
            if (h.flags.IsThereFlow == true && sum(ismember(indx,4)))
                Correlation('Flow',dataPaths(i,1));
                waitbar((7*(i-1)+4)/(7*nFolders),k);
            end
            if (h.flags.IsThereYellow == true && sum(ismember(indx,5)))
                Correlation('Yellow',dataPaths(i,1));
                waitbar((7*(i-1)+5)/(7*nFolders),k);
            end
            if (h.flags.IsThereRed == true && sum(ismember(indx,6)))
                Correlation('Red',dataPaths(i,1));
                waitbar((7*(i-1)+6)/(7*nFolders),k);
            end
            if (h.flags.IsThereGreen == true && sum(ismember(indx,7)))
